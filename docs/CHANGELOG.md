...
- 저장소 첫 화면의 파일 목록을 줄이기 위해 보조 스크립트는 `scripts/`, 상세 문서는 `docs/`와 `.github/`로 모았습니다.
- 설치 진입점은 최상위 `setup.sh`에 유지합니다. 앱 기능과 버전은 바꾸지 않았으며, 공개된 `v2.16.5` 태그와 릴리스 ZIP의 기존 경로도 유지합니다.
