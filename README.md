...
![메뉴바를 걷고 쉬는 Catoshi — 같은 메뉴바의 고양이 주변 확대와 전체 모습](docs/images/catoshi-menubar.gif)

*12초 반복 · 앱의 실제 동작을 가상 시세로 재현했습니다. 고양이 주변 확대와 전체 메뉴바는 같은 장면입니다.*
...
아래 화면은 현재 앱의 실제 SwiftUI/AppKit 컴포넌트를 가상 예시 데이터로 렌더링했습니다. 설명용 제목·여백을 더했으며, 실제 시장 캡처·실제 사용자 성과가 아닙니다.

### 고양이가 사는 메뉴바

고양이는 메뉴바의 Catoshi 영역을 산책하고, 잠깐 멈춰 주변을 살피고, 앉거나 식빵 자세로 쉽니다. 아래는 같은 메뉴바 장면의 **고양이 주변 확대와 전체 모습**입니다.

![메뉴바 속 고양이를 중심으로 본 Catoshi — 같은 장면의 확대와 전체 모습, 가상 예시](docs/images/catoshi-menubar.png)
